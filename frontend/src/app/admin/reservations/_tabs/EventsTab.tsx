...
export function EventsTab({
  ...
  const saveEditEvent = async () => {
    ...
    try {
      ...
      // 時間・定員・ステータス
      await adminPatch(`/events/${editEvent.id}/time`, {
    ...
    <div className="bg-white rounded-3 shadow-sm p-4">
      ...
      {editEvent && (
        ...
        >
          ...
          >
            <h3 style={{ fontSize: "16px", fontWeight: 700, marginBottom: "16px" }}>イベント編集（#{editEvent.id}）</h3>
      ...
      {/* イベント作成ボタン */}
      <div className="mb-3">
        <button type="button" className="btn btn-primary" onClick={() => setCreateEventModalOpen(true)}>
          イベントを作成
        </button>
      </div>
              ...
              <tr>
                <th style={{ width: "36px" }}>
                  <input
                    ...
                    onChange={() => (selectedIds.size === events.length ? deselectAll() : selectAll())}
                  />
                </th>
