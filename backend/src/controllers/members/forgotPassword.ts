...
export default async function membersForgotPassword(req: Request, res: Response, _next: NextFunction): Promise<void> {
  ...
  const [rows] = await pool.query("SELECT id, email FROM users WHERE email = ? AND status = 'active' LIMIT 1", [email]);
