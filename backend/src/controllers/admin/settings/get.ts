...
export default async function getSettings(req: Request, res: Response, _next: NextFunction): Promise<void> {
  const corporationId = req.session?.account?.corporationId;
  if (corporationId == null) {
    unauthorized(res);
    return;
  }
  const [rows] = await pool.query("SELECT name FROM corporations WHERE id = ?", [corporationId]);
