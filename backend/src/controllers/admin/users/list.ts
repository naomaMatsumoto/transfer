...
export default async function listUsers(
  ...
  const limit = Math.min(MAX_LIMIT, Math.max(1, parseInt(String(limitStr ?? String(DEFAULT_LIMIT)), 10) || DEFAULT_LIMIT));
  ...
  if (keyword) {
    ...
    conditions.push("(name LIKE ? OR furigana LIKE ? OR email LIKE ? OR address LIKE ? OR phone LIKE ? OR course_type LIKE ?)");
