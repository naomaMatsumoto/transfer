...
async function createReservation(req, res, _next) {
    ...
    try {
        ...
        const [eventRows] = await conn.query("SELECT e.id, e.capacity, e.status, COALESCE(SUM(CASE WHEN r.status IN ('booked','attended') THEN 1 ELSE 0 END), 0) AS reserved_count FROM events e LEFT JOIN reservations r ON r.event_id = e.id WHERE e.id = ? GROUP BY e.id FOR UPDATE", [eventId]);
